...
class CampusConnect(toga.App):
    ...
    def load_main_interface(self):
        try:
            ...
            tabs_list = [
                ("Home", feed.container), ("Reels", reels.container), ("Chat", chat.container),
                ("Groups", groups.container), ("Events", events.container),
                ("People", people.container), ("Profile", profile.container)
            ]

            self.tabs = toga.OptionContainer(style=Pack(flex=1))
            for label, widget in tabs_list:
                self.tabs.add(label, widget)
